...
source_y = - (m1_q - m2_p) * np.cos(m_theta) - m1_p * np.cos(m_theta * 3) 
source_z = (m1_q - m2_p) * np.sin(m_theta) + m1_p * np.sin(m_theta * 3)

scr_y = - m2_q * np.cos(m_theta)
scr_z = - m2_q * np.sin(m_theta)
...
def build_beamline():
    ...
    beamLine.mirror_xrt = roes.EllipticalMirrorParam(
        bl=beamLine,
        name="M1",
        center=[0, - (m1_q - m2_p) * np.cos(m_theta), (m1_q - m2_p) * np.sin(m_theta)],
        pitch=m_theta,
    ...
    beamLine.mirror_xmf = roes.ConvexEllipticCylindricalMirrorXMF(
        bl=beamLine,
        name="M2",
        center=[0, 0, 0],
        pitch=m_theta,
...
def run_process(beamLine):
    ...
    M1Param01beamGlobal01, M1Param01beamLocal01 = beamLine.mirror_xrt.reflect(
        beam=geometricSource01beamGlobal01)

    M2Param01beamGlobal01, M2Param01beamLocal01 = beamLine.mirror_xmf.reflect(
