...
def build_beamline(field_z = 0e-3, scr_offset = 0e-3): # field size in z direction
    ...
    scr_y = (m1_q + scr_offset) * np.cos(m1_theta)
    scr_z = (m1_q + scr_offset) * np.sin(m1_theta)
    ...
    beamLine.geometricSource = rsources.GeometricSource(
        ...
        dxprime=src_dxprime,
        dzprime=src_dzprime,
        nrays=500_000,  # increase the number of rays
    )
...
def run_process(beamLine):
    ...
    screen01beamLocal01 = beamLine.screen.expose(beam=mrrorParam01beamGlobal01)
...
def main():
    ...
    fig_path = os.path.join(
        script_dir, "XRF Reverse Confocal Microscope 3-mm-wd HKB-Screen-offset.png"
    )
